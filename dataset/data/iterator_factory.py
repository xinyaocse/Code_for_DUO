...
def get_hmdb51(data_root=None,
    ...
    train = VideoIter(video_prefix=os.path.join(data_root, 'raw-data'),
                      txt_list=os.path.join('./dataset/HMDB51', 'raw', 'list_cvt', 'hmdb51_split1_train.txt'),
    ...
    val   = VideoIter(video_prefix=os.path.join(data_root, 'raw-data'),
                      txt_list=os.path.join('./dataset/HMDB51', 'raw', 'list_cvt', 'hmdb51_split1_test.txt'),
                      sampler=val_sampler,
                      force_color=True,
                      video_transform=transforms.Compose([
                                         transforms.Resize((128, 171)),
...
def get_ucf101(data_root=None,
    ...
    train_sampler = sampler.RandomSampling(num=clip_length,
                                           ...
                                           seed=(seed+0))
    train = VideoIter(video_prefix=os.path.join(data_root, 'raw-data'),
                      txt_list=os.path.join('./dataset/UCF101', 'raw', 'list_cvt', 'trainlist01.txt'),
                      sampler=train_sampler,
                      force_color=True,
                      video_transform=transforms.Compose([
                                         transforms.RandomScale(make_square=True,
    ...
    val_sampler   = sampler.SequentialSampling(num=clip_length,
                                               ...
                                               shuffle=True)
    val   = VideoIter(video_prefix=os.path.join(data_root, 'raw-data'),
                      txt_list=os.path.join('./dataset/UCF101', 'raw', 'list_cvt', 'testlist01.txt'),
                      sampler=val_sampler,
                      force_color=True,
                      video_transform=transforms.Compose([
                                          transforms.Resize((128, 171)),
...
def get_kinetics(data_root='./dataset/Kinetics',
    ...
    train_sampler = sampler.RandomSampling(num=clip_length,
                                           ...
                                           seed=(seed+0))
    train = VideoIter(video_prefix=os.path.join(data_root, 'raw-data'),
                      txt_list=os.path.join('./dataset/KINETICS', 'raw', 'list_cvt', 'kinetics_train_w-missed-v1_avi.txt'),
                      sampler=train_sampler,
                      force_color=True,
                      video_transform=transforms.Compose([
                                         transforms.RandomScale(make_square=True,
    ...
    val_sampler   = sampler.SequentialSampling(num=clip_length,
                                               ...
                                               shuffle=True)
    val   = VideoIter(video_prefix=os.path.join(data_root, 'raw-data'),
                      txt_list=os.path.join('./dataset/KINETICS', 'raw', 'list_cvt', 'kinetics_val_w-missed-v1_avi.txt'),
                      sampler=val_sampler,
                      force_color=True,
                      video_transform=transforms.Compose([
