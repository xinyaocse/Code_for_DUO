...
train_pipeline = [
    dict(type='SampleFrames', clip_len=16, frame_interval=1, num_clips=1),  #changed
    dict(type='FrameSelector'),
    dict(type='RandomResizedCrop'),
    dict(type='Resize', scale=(112, 112), keep_ratio=False),  # changed
...
test_pipeline = [
    ...
    dict(type='FrameSelector'),
    dict(type='Resize', scale=(128, -1)),  # (-1, 256)), # we need 128
    dict(type='ThreeCrop', crop_size=112),  # 224), # we need 112
    dict(type='Normalize', **img_norm_cfg),
    dict(type='FormatShape', input_format='NCTHW'),
    dict(type='Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type='ToTensor', keys=['imgs', 'label'])
...
log_level = 'INFO'
work_dir = f'./work_dirs/tpn_imagenet_pretrained_slowonly_r50_8x8x1_150e_ucf101_{split}_rgb'  # changed
load_from = None
resume_from = None
