...
class RawframeRGBFlowDataset(BaseDataset):
    """RawframeRGBFlow dataset for action recognition.
    ...
    def __init__(self,
                 ...
                 data_prefix=None,
                 test_mode=False,
                 filename_tmpl='frame{:06}.jpg',##'img_{:05}.jpg',
                 with_offset=False,
                 multi_class=False,
                 num_classes=None,
                 start_index=1,
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                line_split = line.strip().split() # v_BoxingPunchingBag_g12_c03 160 16
                video_info = {}
                idx = 0
                # idx for frame_dir
                ...
                if self.with_offset:
                    ...
                    video_info['total_frames'] = int(line_split[idx + 1])-1# added by Shasha
                    idx += 2
                else:
                    # idx for total_frames
                    video_info['total_frames'] = int(line_split[idx])-1# added by Shasha
                    idx += 1
                # idx for label[s]
    ...
    def prepare_train_frames(self, idx):
        ...
        results_flow = copy.deepcopy(results)
        results_flow['modality'] = 'RGB'


        return self.pipeline(results), self.pipeline(results_flow) #self.pipeline(results) #

    def prepare_test_frames(self, idx):
        #print('preparing_test_frames')
        ...
        if self.sample_by_class:
            ...
            results = copy.deepcopy(self.video_infos[idx])
        results['filename_tmpl'] = self.filename_tmpl
        # results['filename_flow'] = self.filename_flow # do we need this?
        results['modality'] = self.modality
        results['start_index'] = self.start_index
