    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                #print(directory)
                frame_dir_split = directory.split('/')
                if 'UCF' in directory:
                    frame_dir_split[-2] = 'tvl1_flow/u'
                    filename = filename_tmpl.format(frame_idx)
                elif 'JESTER' in directory:
                    frame_dir_split[-2] = 'tvl1'
                    filename = 'flow_x_'+filename_tmpl.format(frame_idx)
                else:
                    print('Do not support such dataset')
                    raise
                x_filedir = '/'.join(frame_dir_split)
                x_filepath = osp.join(x_filedir, filename)
                if 'UCF' in directory:
                    frame_dir_split[-2] = 'tvl1_flow/v'
                    filename = filename_tmpl.format(frame_idx)
                elif 'JESTER' in directory:
                    frame_dir_split[-2] = 'tvl1'
                    filename = 'flow_x_'+filename_tmpl.format(frame_idx)
                    ...
                    print('Do not support such dataset')
                    raise
                y_filedir = '/'.join(frame_dir_split)
                y_filepath = osp.join(y_filedir, filename)
                x_img_bytes = self.file_client.get(x_filepath)
                x_frame = mmcv.imfrombytes(x_img_bytes, flag='grayscale')
                y_img_bytes = self.file_client.get(y_filepath)
                y_frame = mmcv.imfrombytes(y_img_bytes, flag='grayscale')
